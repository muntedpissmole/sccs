...
def _demo_mode_enabled() -> bool:
    try:
        ...
        from modules.demo import is_demo_enabled

        return bool(is_demo_enabled(config))
    except Exception:
        return False


def _demo_wifi_networks(*, connected_ssid: str = "Singularity") -> list[dict[str, Any]]:
    """Example SSIDs for the demo UI when nmcli is missing or empty."""
    # Stable-ish list with a few randomised personal hotspots
    hotspots = [
        "iPhone",
        "Joe's iPhone",
        "Sarah’s iPhone",
    ...
    pick = random.sample(hotspots, k=min(3, len(hotspots)))
    networks: list[dict[str, Any]] = [
        {
            "ssid": "Singularity",
            "signal": 92,
            "security": "WPA2",
            "in_use": connected_ssid == "Singularity",
            "saved": True,
            "band": "5 GHz",
        },
        {
            ...
            "signal": 78,
            "security": "WPA2",
            "in_use": connected_ssid == "CamperNet",
            "saved": True,
            "band": "2.4 GHz",
        },
        {
    ...
    for i, name in enumerate(pick):
        networks.append({
            "ssid": name,
            "signal": random.randint(28, 72),
            "security": "WPA2",
            "in_use": False,
            "saved": False,
            "band": "5 GHz" if i % 2 == 0 else "2.4 GHz",
        })
    # Sort by signal desc like a real scan
    networks.sort(key=lambda n: int(n.get("signal") or 0), reverse=True)
    return networks


# In-memory Wi-Fi theatre for demo mode — never calls nmcli / never changes host net.
_DEMO_WIFI_STATE: dict[str, Any] = {
    "ssid": "Singularity",
    "connected": True,
    "preferred": "wifi",
    "ip": "10.10.10.42",
...
def _demo_wifi_status() -> dict[str, Any]:
    """Self-contained Wi-Fi status for demo installs (never touches system Wi-Fi)."""
    connected = bool(_DEMO_WIFI_STATE.get("connected"))
    connected_ssid = _DEMO_WIFI_STATE.get("ssid") if connected else None
    networks = _demo_wifi_networks(connected_ssid=connected_ssid or "Singularity")
    if not connected:
        for net in networks:
            net["in_use"] = False
    signal = 92
    if connected_ssid:
        signal = next(
            (int(n.get("signal") or 70) for n in networks if n.get("ssid") == connected_ssid),
            70,
        )
    prefer = _DEMO_WIFI_STATE.get("preferred") or "wifi"
    status = {
        "available": True,
        "iface": "wlan0",
        ...
        "security": "WPA2" if connected else None,
        "ip": _DEMO_WIFI_STATE.get("ip") if connected else None,
        "networks": networks,
        "error": None,
        "source": "demo",
        "uplink": {
            ...
            "usb": None,
            "active": "wifi" if connected and prefer == "wifi" else (prefer if connected else None),
            "preferred": prefer,
            "wifi_online": connected,
            "usb_online": False,
        },
    }
    return status
...
def get_wifi_status() -> dict[str, Any]:
    # Demo: pure in-memory UI — no nmcli, no route metrics, no host network changes.
    if _demo_mode_enabled():
        return _demo_wifi_status()
...
def scan_wifi_networks() -> dict[str, Any]:
    if _demo_mode_enabled():
        # Rescan only reshuffles demo list — never calls nmcli device wifi rescan.
        connected = bool(_DEMO_WIFI_STATE.get("connected"))
        ssid = _DEMO_WIFI_STATE.get("ssid") if connected else None
        networks = _demo_wifi_networks(connected_ssid=ssid or "Singularity")
        if not connected:
            for net in networks:
                net["in_use"] = False
        return {
...
def connect_wifi(ssid: str, password: str | None = None) -> dict[str, Any]:
    ...
    if _demo_mode_enabled():
        # UI-only connect — never runs nmcli connection up / device wifi connect.
        _DEMO_WIFI_STATE["ssid"] = ssid
        _DEMO_WIFI_STATE["connected"] = True
        _DEMO_WIFI_STATE["ip"] = "10.10.10.42"
        status = _demo_wifi_status()
        return {
            "ok": True,
...
def disconnect_wifi() -> dict[str, Any]:
    if _demo_mode_enabled():
        # UI-only disconnect — never runs nmcli device disconnect.
        _DEMO_WIFI_STATE["connected"] = False
        _DEMO_WIFI_STATE["ssid"] = None
        status = _demo_wifi_status()
        return {
            "ok": True,
...
def set_preferred_uplink(prefer: str) -> dict[str, Any]:
    ...
    if _demo_mode_enabled():
        # Save preference for the UI only — never rewrites route metrics via nmcli.
        _DEMO_WIFI_STATE["preferred"] = prefer
        return {
            "ok": True,
            "message": f"Preferred internet set to {prefer_name}",
            "status": _demo_wifi_status(),
            "source": "demo",
        }
