...
def inject_nav():
    ...
    return {
        ...
        "demo_mode": is_demo_enabled(config),
    }
...
def _screens_list(*, probe: bool = False) -> list:
    ...
    demo = is_demo_enabled(config)
    screens = []
    for name, conf in runtime.compiled.screens.items():
        conn = None
        # Demo: always use simulated connectivity (no SSH), even for list polls.
        if probe or demo:
...
def api_system_shutdown():
    ...
    Demo mode never powers off the host — this may be a shared server.
    """
    from modules.demo import is_demo_enabled

    if is_demo_enabled(config):
        logger.warning(
            "System shutdown requested in DEMO mode — host power-off suppressed"
        )
        try:
            toast_manager.info(
                "Demo mode: shutdown simulated (host not powered off)",
                title="Shutdown",
                duration=5000,
            )
        except Exception:
        ...
        return jsonify({
            "ok": True,
            "message": "Demo mode — shutdown simulated (host not powered off)",
            "demo": True,
        })
...
def cleanup():
    ...
    if demo_activity:
        try:
            ...
            logger.debug(f"Demo activity stop: {e}")
...
def _startup():
    ...
    demo_mode = is_demo_enabled(config)
    if demo_mode:
        logger.info("Starting SCCS lighting backend… [DEMO MODE — dummy data]")
    else:
    ...
    if demo_mode:
        try:
            install_demo_screens(runtime)
        except Exception as e:
            logger.error(f"Demo screens setup failed: {e}")
    ...
    if not demo_mode and getattr(gps_module, "serial", None):
        gps_module.start_reader()

    try:
        if demo_mode:
            victron_module = DemoVictronManager(
                socketio, config, phase_manager=phase_manager
            )
        else:
            from modules.victron import VictronManager

            victron_module = VictronManager(
                socketio, config, phase_manager=phase_manager
            )
    ...
    if demo_mode:
        try:
            demo_activity = DemoActivitySimulator(runtime, config)
            demo_activity.start()
        except Exception as e:
            logger.error(f"Demo activity simulator failed: {e}")
            demo_activity = None
        logger.info("SCCS lighting backend ready (demo mode)")
    else:
