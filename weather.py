...
# Alexandra, Victoria — default and demo weather pin
_DEFAULT_LAT = -37.191
_DEFAULT_LNG = 145.711
_ALEXANDRA_LAT = -37.1914
_ALEXANDRA_LNG = 145.7100

_CACHE: dict[str, Any] = {"ts": 0.0, "payload": None, "lat": None, "lng": None}
_CACHE_TTL = 300.0
...
def _coords() -> tuple[float, float]:
    # Demo branch: always scrape Open-Meteo / wttr for Alexandra, Victoria.
    if _demo_mode_enabled():
        return _ALEXANDRA_LAT, _ALEXANDRA_LNG
...
def get_weather_status() -> dict[str, Any]:
    now = time.time()
    lat, lng = _coords()
    # Bust cache when location changes (e.g. after switching to Alexandra pin)
    cached_lat = _CACHE.get("lat")
    cached_lng = _CACHE.get("lng")
    if (
        _CACHE["payload"]
        and now - _CACHE["ts"] < _CACHE_TTL
        and cached_lat is not None
        and abs(float(cached_lat) - lat) < 1e-4
        and abs(float(cached_lng) - lng) < 1e-4
    ):
    ...
    if _demo_mode_enabled():
        payload["location_label"] = "Alexandra, Victoria"

    _CACHE.update({"ts": now, "payload": payload, "lat": lat, "lng": lng})
    return dict(payload)