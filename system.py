...
_DEMO_CORE: dict[str, Any] = {
    ...
    # Generic fallback only — never claim a Pi when the host is something else.
    "model": "SCCS Core",
    "os": "Linux",
...
def _read_dmi_model() -> str | None:
    ...
    for key, attr in (
        ...
        try:
            ...
            continue
        if not raw or raw.lower() in {"none", "default string", "to be filled by o.e.m.", "system product name"}:
            continue
        if attr == "vendor":
            vendor = raw
        elif attr == "product":
...
def _read_model() -> str | None:
    # Prefer DMI on generic Linux hosts (NUC / GMKtec / etc.) so we don't fall
    # through to a Pi-shaped demo default when device-tree is absent.
    dmi = _read_dmi_model()
    for path in ("/proc/device-tree/model", "/sys/firmware/devicetree/base/model"):
        try:
            raw = Path(path).read_bytes().decode("utf-8", errors="ignore").strip("\x00")
            if raw:
                # If DMI also names a vendor product, prefer it when device-tree
                # looks like a stock Pi string on a non-Pi chassis (rare), else Pi.
...
def get_core_info() -> dict[str, Any]:
    ...
    if model:
        ...
        # Never leave the RPi placeholder when we couldn't detect hardware.
        core["model"] = f"SCCS Demo Host ({platform.machine() or 'linux'})"
    if cores:
...
def get_system_status() -> dict[str, Any]:
    ...
    # Demo branch: SCCS is always fully connected — no offline hardware tiles.
